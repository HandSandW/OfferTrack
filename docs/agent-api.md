...
本页描述阶段 3 C 快照、JSON CLI、本地 stdio MCP 和受控写入。已提供自动按内容刷新和新鲜度检查。代码契约独立于桌面 IPC，字段使用 snake_case。阶段 4 B 数据库 schema 已追加至 11，仓库及 Agent 契约格式仍为 1；附件重命名及单附件回收站仅由桌面提供，Agent 不增加文件修改/恢复/删除能力，待处理附件文件日志会阻止官方 Agent 写入。
...
- 不调用程序、直接读取文件：在设置的“本地 Agent 访问”中点击“生成 Agent 只读快照”，将返回的目录交给 Agent。
- 快照仅代表其生成时刻。桌面连接后自动检查并按需生成，仍可主动强制生成；旧代和失败暂存均保留。旧代可能仍包含后来已删除的信息，不上传 GitHub，不当作当前状态或完整备份。
...
目录为 `agent-access/snapshot-<UTC时间>-<UUID>/`。先写 `.pending-<UUID>`，所有文件刷盘后以验证身份的目录重命名一次发布。不覆盖任何旧代、用户说明或同名目录，无永久删除行为。仅可写会话可生成（桌面或已授权写后的 CLI/MCP）；不修改投递数据，不触发业务备份。发布后单独事务保存派生检查点，失败不撤销文件发布。发布失败保留暂存。
...
读取者只选择 `snapshot-...` 完成目录，忽略 `.pending-*`。确认 manifest 的仓库 ID 与当前 warehouse.json 匹配、版本受支持，并验证所有列出的文件大小/哈希后读取同一代的文件。多代时以 manifest 的 generated_at_utc 最新者为候选，同时间按目录名排序；系统时钟异常时不能据目录名断言新鲜度，改用 CLI。不能混用不同代的 JSONL。当前不维护可被逐文件覆盖的“current”副本，也不自动清理历史。优先用下述 snapshot_status 获取内容校验过的代，避免以系统时钟先后判断。

## 自动刷新与新鲜度

桌面连接仓库后、每分钟、窗口焦点恢复及相关编辑/文件索引更新结束后检查，编辑合并约 1 秒。可写仓库按内容变化新建快照，只读仅检查；应用关闭后无后台任务。没有变化的重复扫描/设置修改不生成新代，手动“生成”仍强制新代。索引以外的文件变化不会由快照检查偷偷扫描。
...
派生检查点以 version=1 保存在 settings.agent_snapshot_v1，绑定仓库身份、代相对路径和清单哈希；不向 Agent 提供修改它的接口。没有检查点的旧代保留，但新鲜度报告 missing，下一次可写自动检查另建新代。未知版本/损坏检查点拒绝覆盖并报错；不要直接改数据库来强制通过。

指纹排除生成时钟，包含业务时间戳、版本、长文本、索引与 Schema。迁移/恢复后重新核验文件和内容；数据库单独恢复没有快照文件且新仓库 ID 不沿用旧状态。根 AGENTS.md 仍不覆盖。已发布但检查点未保存时独立报告 published=true/state=error，保留文件；桌面本次监测暂停自动重试，主动检查或重连后重试，防止每分钟累积重复代。旧代/失败暂存可能含私人旧数据，不自动清理。
