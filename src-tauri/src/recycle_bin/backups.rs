//! Explicitly confirmed cleanup of UUID directories in recycle-bin/backups only.
use crate::{
    database_backup,
...
pub fn purge(
    ...
    for item in current {
        let path = session.root().join("recycle-bin/backups").join(&item.id);
        match super::remove_tree_in_area(
            session.root(),
            &path,
            super::TrashArea::Backups,
            Some(&item.identity),
        ) {
    ...
    // Directory presence is authoritative, including partial failures or process exit.
    // No separate SQL catalogue to falsely mark a partially removed backup complete.
    Ok(result)
}
