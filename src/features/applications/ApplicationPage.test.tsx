...
describe("application management", () => {
  it("creates without opening detail and opens the independent detail window explicitly", async () => {
    let created = false;
    const record = applicationFixture();
    vi.mocked(desktopApi.listApplications).mockImplementation(() =>
      Promise.resolve(created ? [record] : []),
    );
    const create = vi
    ...
    expect(desktopApi.setApplicationDetailTarget).toHaveBeenCalledWith(
      record.id,
      false,
    );
    fireEvent.click(screen.getByRole("button", { name: "详情" }));
    expect(desktopApi.setApplicationDetailTarget).toHaveBeenLastCalledWith(
      record.id,
      true,
    );
  ...
  beforeEach(() => {
    ...
    vi.spyOn(desktopApi, "setApplicationDetailTarget").mockResolvedValue();
  ...
  it("restores the default view and page size on mount and does not reset them on selection", async () => {
    ...
    await screen.findByText("示例公司");
    expect(screen.getByLabelText("自定义每页条数")).toHaveValue(100);
    expect(screen.getByLabelText("搜索投递")).toHaveValue("示例");
    expect(screen.getByLabelText("已保存视图")).toHaveValue("saved");
    fireEvent.change(screen.getByLabelText("每页条数快捷选择"), {
      target: { value: "20" },
    });
    fireEvent.click(screen.getByText("示例公司"));
    expect(screen.getByLabelText("每页条数快捷选择")).toHaveValue("20");
    ...
    fireEvent.change(screen.getByLabelText("自定义每页条数"), {
      target: { value: "37" },
    });
    fireEvent.click(screen.getByRole("button", { name: "设为默认" }));
    await waitFor(() =>
      expect(desktopApi.setApplicationPageSize).toHaveBeenLastCalledWith(37),
    );
    fireEvent.change(screen.getByLabelText("自定义每页条数"), {
      target: { value: "501" },
    });
    expect(screen.getByText("请输入 1–500 的整数。")).toBeInTheDocument();
  ...
  it("shows the current record label and delegates history to the independent detail", async () => {
  ...
  it("updates independent detail target when table focus changes", async () => {
    ...
    expect(desktopApi.setApplicationDetailTarget).toHaveBeenNthCalledWith(
      1,
      "fixture-application",
      false,
    );
  ...
  it("does not mount editable detail controls in the table window", async () => {
    render(<ApplicationPage scope="active" writable onError={onError} />, {
      wrapper: DraftGuardProvider,
    });
    fireEvent.click(await screen.findByText("示例公司"));
